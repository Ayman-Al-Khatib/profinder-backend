...
const {
  CompanyApplication,
  employeeRanges,
} = require('../../../models/company_applications_model');
...
exports.createOneValidation = [
  body('name')
    .notEmpty()
    .withMessage($.company_name_is_required)
    .bail()
    .isString()
  ...
  body('email')
    ...
    .bail()
    .isEmail()
    .withMessage($.company_email_must_be_valid)
    .bail()
    .matches(/^([\w-.]+@([\w-]+\.)+[\w-]{2,4})?$/)
    ...
    .custom(async value => {
      ...
      const application = await CompanyApplication.findOne({ email: value });
      if (
        (company && !company.deleted_at) ||
        (application && application.verification.status === 'pending' && !application.deleted_at)
      ) {
        throw new ApiError($.the_company_email_should_be_unique, 400);
      }
      return true;
    }),
  ...
  body('industry')
    .notEmpty()
    .withMessage($.company_industry_is_required)
    .bail()
    .isString()
  ...
  body('size')
    .exists()
    .withMessage($.company_size_is_required)
    .isObject()
    .withMessage($.company_size_must_be_an_object)
    .custom(value => {
      if (!value || !value.min || !value.max) {
        throw new ApiError($.company_size_is_not_supported, 400);
      }
  ...
  body('description')
    .notEmpty()
    .withMessage($.company_description_is_required)
    .bail()
    .isString()
  ...
  body('website')
    ...
    .matches(
      ...
      /^((https?:www\.)|(https?:\/\/)|(www\.))?[\w/\-?=%.][-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9]{1,6}(\/[-a-zA-Z0-9()@:%_\+.~#?&\/=]*)?$/,
    )
    .withMessage(
  ...
  body('location')
    .notEmpty()
    .withMessage($.company_location_is_required)
    .bail()
    .isString()
...
exports.updateOne = [
  ...
  body('size')
    ...
    .custom(value => {
      ...
      if (!isValidRange) throw new ApiError($.company_size_is_not_supported, 400);
